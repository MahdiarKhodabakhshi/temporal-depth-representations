...
def _parse_mteb_dataset_config_overrides():
    """
    Parse optional overrides from:
    MTEB_DATASET_CONFIG_OVERRIDES="mteb/amazon_counterfactual:en,mteb/foo:bar"
    """
    raw_overrides = os.getenv("MTEB_DATASET_CONFIG_OVERRIDES", "")
    overrides = {}
    ...
    for item in raw_overrides.split(","):
        ...
        if not item or ":" not in item:
            continue
        dataset_name, config_name = item.split(":", 1)
        dataset_name = dataset_name.strip()
        config_name = config_name.strip()
        if dataset_name and config_name:
            overrides[dataset_name] = config_name

    return overrides


def _infer_cached_config_from_error(error_message):
    marker = "configurations in the cache:"
    if marker not in error_message:
        return None

    config_line = error_message.split(marker, 1)[1].splitlines()[0]
    configs = [cfg.strip() for cfg in config_line.split(",") if cfg.strip()]
    if not configs:
        return None

    # English tasks in this benchmark should prefer the english cached config.
    if "en" in configs:
        return "en"
...
def _load_mteb_dataset_split(dataset_name, split):
    overrides = _parse_mteb_dataset_config_overrides()
    candidates = []
    if dataset_name in overrides:
        candidates.append(overrides[dataset_name])
    candidates.append(None)  # Try default/no-config path as fallback.

    idx = 0
    last_error = None
    while idx < len(candidates):
        config_name = candidates[idx]
        idx += 1
        try:
            if config_name is None:
                dataset_dict = load_dataset(dataset_name, trust_remote_code=True)
            else:
                print(f"Loading {dataset_name} with config '{config_name}'")
                dataset_dict = load_dataset(dataset_name, config_name, trust_remote_code=True)
            ...
            last_error = e
            if config_name is None:
                inferred_config = _infer_cached_config_from_error(str(e))
                if inferred_config is not None and inferred_config not in candidates:
                    print(f"Detected multiple cached configs for {dataset_name}. Retrying with config '{inferred_config}'")
                    candidates.append(inferred_config)
...
def find_data_key_in_examples(examples):
    if "text" in examples:
        ...
        return "sentence1"
    elif "sent1" in examples and "sent2" in examples:
        return "sent1"
    else:
...
def get_dataloader(
    ...
    if dataset_name == 'wikitext':
        ...
        try:
            dataset = _load_mteb_dataset_split(dataset_name, split)
        except Exception as e:
